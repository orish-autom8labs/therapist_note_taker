#!/bin/bash
# Deploy Note Taker frontend to Google Cloud Run
set -e  # Exit on error
...
BACKEND_URL="https://note-taker-backend-1049928242674.us-central1.run.app"
IMAGE_NAME="${REGION}-docker.pkg.dev/${PROJECT_ID}/cloud-run-source-deploy/${SERVICE_NAME}"

echo -e "${BLUE}=== Note Taker Frontend Deployment ===${NC}"
echo ""
